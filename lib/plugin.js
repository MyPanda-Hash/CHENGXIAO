  ...
  '~standard': {
    ...
    validate(input) {
      ...
      if (raw.relayUrl === undefined) {
        // Enabling the relay without naming one routes through the official
        // public relay, so a fresh install works with zero infrastructure.
        value.relayUrl = value.relayEnabled === true ? OFFICIAL_RELAY_URL : undefined;
      } else if (typeof raw.relayUrl !== 'string' || !/^https?:\/\/\S+$/u.test(raw.relayUrl.trim())) {
        issues.push({ message: `relayUrl must be an http(s) URL, got ${JSON.stringify(raw.relayUrl)}` });
      } else {
        value.relayUrl = raw.relayUrl.trim().replace(/\/+$/u, '');
      }

      if (raw.relayDeviceId === undefined) {
        value.relayDeviceId = undefined;
      } else if (typeof raw.relayDeviceId !== 'string' || raw.relayDeviceId.trim() === '') {
        issues.push({ message: 'relayDeviceId must be a non-empty name' });
      } else {
        value.relayDeviceId = raw.relayDeviceId.trim();
      }
...
export async function apply(ctx, config) {
  ...
  const service = await createPeerService({
    ...
    mcp: McpClient,
    mountContext: ctx,
    // An ask may run as long as the worker's task timeout allows, while the
    // client library defaults to one minute; the mount must outwait the task
    // so the worker's own (informative) timeout error wins over a bare
    // client-side "request timed out" that hides a still-running task.
    toolCallTimeoutMs: config.taskTimeoutMs + 30_000,
    ...(config.addresses !== undefined && { addresses: config.addresses }),
    // The relay connection is opt-in: nothing changes for profiles that do
    // not name one, and a named relay that cannot be reached stops startup
    // rather than leaving pairing half-working.
    ...(config.relayEnabled === true && {
      relay: {
        url: config.relayUrl,
        enabled: true,
        ...(config.relayDeviceId !== undefined && { deviceId: config.relayDeviceId }),
      },
    }),
    log,
  });
