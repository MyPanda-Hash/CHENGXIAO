...
> 不是安装前提。
>
> **跨网络也可以不用 Tailscale**：部署一台自托管中继后，两台机器改用
> `relayEnabled: true` + `relayUrl` 配置（见 README「中继模式」），双方都出站连接
...
**插件本身不关心你在哪个网段**：它不按来源 IP 或网段做任何放行/拒绝，只看配对码和凭据。
只要 TCP 能到达 7331，跨网络照样能配对。（调用方地址唯一的用途是给失败次数计数，
见下面的限流说明；它不参与"允不允许连"的判断。）

**但"两台机器各自在 NAT 后面"这种情况，光靠插件不行** —— 地址是私有的，谁也路由不到谁。
三条路，成本从低到高：

| 做法 | 改动 | 代价 |
|---|---|---|
| **① 组一个虚拟局域网**（Tailscale / ZeroTier / WireGuard） | 零代码 | 两台都装、都登同一个网络 |
| **② 端口映射**（把干活端的 7331 暴露到公网） | 零代码 | 安全面变大，见下 |
| **③ 在插件里做中继/打洞** | 要写新功能 | 需一台公网服务器 + 协议改动 |

### ① 虚拟局域网（推荐）

两台机器装上 Tailscale 之类的组网工具后会各拿到一个虚拟地址（如 `100.x.y.z`），
互相能通，插件直接可用。**但有一个坑**：插件默认会跳过这类网卡 ——
`service.js` 里有一份降权名单：
...
所以在这种网络里**通常必须手动锁定要广播的地址**，否则配对码里写的还是局域网地址，
对端永远连不上。实测（`chooseAdvertisedAddress`，注入网卡列表）：

| 网卡情况 | 自动选出来的 | 结论 |
|---|---|---|
| Tailscale + 真实局域网 | `192.168.0.238`（局域网） | ❌ 对面在别的网络时连不上，**必须锁定** |
| Tailscale + 真实局域网，且锁定 | `100.101.102.103` | ✅ |
| **只有** Tailscale（没有别的非回环网卡） | `100.101.102.103` | ✅ 这种情况不用锁 |
| 只有 ZeroTier | `10.147.20.30` | ✅ 不用锁 |

也就是说：降权不等于排除 —— 只有"除了它没别的可选"时它才会被选中。而装了组网工具的机器
**同时还有局域网地址是常态**，所以照例请显式锁定：

```yaml
        host: 0.0.0.0            # 默认值，绑所有网卡，虚拟网卡也能连进来
        addresses:
          - '100.x.y.z'          # 换成那台机器在虚拟局域网里的地址
```
...
能不能用取决于公网 IP 是否固定、有没有 CGNAT（大内网就没戏）。

安全上要知道的事（照现状如实说）：

- 配对码一次性、15 分钟有效；`/pair` 有**按来源的**失败次数限制（默认 5 次后 429）；
  配对成功后换发长期凭据，`/mcp` 每次都要凭据。
- 但这些是"够用"级别的防护，**不是为公网暴露设计的**。真要暴露，建议只开很短的时间窗、
  用完立刻关掉映射，或者干脆走 ①。
- 一旦配对成功，对端就能在你白名单目录里以**那台机器自己的 DSH 权限**执行任务 ——
  设计如此，所以暴露前先想清楚 `allowedDirs` 划到哪里。

### ③ 中继/打洞

需要一台公网服务器做会合与转发，涉及协议改动（凭据转发、连接复用、地址协商）。
当前实现里没有，属于新功能。
...
| 动作 | 碰到什么 | 碰不到什么 |
|---|---|---|
| 装插件（dsh plugin add） | 只往 profile 的 node_modules 加一个包、bundle 注册一行 | 不碰任何已有插件；已有插件照常加载 |
| 写 profile（listen / allowedDirs / addresses） | 只动 `dsh-peer-mcp` 那一块 config，**写前自动备份**（`cordis.patch.yml.bak-时间戳`） | 其余条目原样（用宿主 yaml 解析器逐项验收过） |
| 防火墙规则 | 只放行入站 **7331/TCP** 一条 | 不开其它端口，不改已有规则 |
| 重启 DSH | **会中断那台机器上正在跑的 DSH 任务** —— 这是唯一的实际影响，挑空闲时段做 | 磁盘上的文件、项目代码、非 DSH 的服务都不受影响 |
| 对端 `ask` 干活 | 只能在 `allowedDirs` 白名单目录里 | 白名单外的路径直接拒绝（实测返回 `path-not-allowed`） |

**关键控制就是 `allowedDirs`**：不想让对端碰的项目，目录别放进白名单即可——
比如这台机器上有个正在使用的项目，就把它排除在外，对端连读都会被拒。

两个要提前查的点：

1. **7331 被占**：那台机器上如果已有程序在用 7331，插件起不来。跑 `诊断配对问题.ps1`，
   第 3 步会直接报出占用者的进程名。解法是给插件换个端口（profile 里 `port:`），
   **两边不需要同一个端口** —— 配对码里自带端口。
2. **别装多余的东西**：如果那台机器和你在同一个局域网（比如 192.168.0.235 那台），
   它**不需要** Tailscale —— 组网只解决"不同网络"的问题。少装一样就少一样风险。
   Tailscale 若确要装：默认只给自己网的 `100.x.y.z` 加路由，不改默认路由，
   不影响那台机器正常上网和局域网访问。

最坏情况的兜底：把 profile 里 `dsh-peer-mcp` 那一条删掉（或 `disabled: true`）并重启，
插件就完全退场；目录 `node_modules\dsh-peer-mcp` 可一并删除。防火墙规则删掉即恢复原状。
